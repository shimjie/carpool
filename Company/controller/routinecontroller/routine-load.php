<?php
    ...
    FROM(
        ...
        FROM(
            SELECT driver_infor.*, user_detail.perName, user_detail.phone, user_detail.email, user_detail.driver_rate
            FROM driver_infor, user_detail, user_infor
            WHERE driver_infor.user_id = user_infor.id and user_infor.user_name = user_detail.user_name) rs
    ...
    //Get basic data from driver_infor, user_detail and user_infor
    ...
    $result = $mysqli->query($query);
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc())
                ...
                $row["perName"], $row["phone_open"], $row["phone"], $row["email_open"], $row["email"], $row["open_status"]));
        }
